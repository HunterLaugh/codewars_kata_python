...
class solution(object):
	...
	def check(self,x,y,value):	#检查每行、每列及每宫是否有相同项
		...
		for row_n in range(x+1,9):
			for col_n in range(0,9):
				if self.b[row_n][col_n]==0:
					return [row_n,col_n]
		return [-1,0]	#若无下一个未填项，返回-1
		
		
	def try_it(self,x,y):	#主循环
		if self.b[x][y]==0:
			for i in range(1,10):	#从1到10尝试
				self.t+=1
				if self.check(x,y,i):	#符合 行列宫均无重复的
					self.b[x][y]=i		#将符合条件的填入0格
					next_x,next_y=self.get_next(x,y)		#得到下一个0格
					if next_x==-1:	#如果无下一个0格
						return True		#返回True
					else:		#如果有下一个0格，递归判断下一个0直到填满数独
						end=self.try_it(next_x,next_y)
						if not end:		#在递归过程中存在不符合条件的，即使，即使try_it函数返回None的项
							self.b[x][y]=0	#回溯到上一层继续
						else:
							return True
